...
class GuiManager:
    ...
    def render_plot_trajectory_menu(self, solar_system):
        ...
        if self.show_trajectory_menu:
            ...
            # Dropdowns for selecting celestial bodies, organized by categories
            for i in range(2):
                label = f"Body {i+1}:"
                imgui.text(label)
                imgui.same_line()
                    ...
                    for category, bodies in categories.items():
                        if imgui.tree_node(category):
                            for body_name in bodies:
                                ...
                                if selected:
                                    self.handle_trajectory_body_selection(i, body_name, solar_system)
                            imgui.tree_pop()
                    imgui.end_combo()
    ...
    def handle_trajectory_body_selection(self, index, body_name, solar_system):
